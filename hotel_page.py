"""Hotel availability checker UI (embedded in main app)."""
...
def _render_result(result: AvailabilityResult) -> None:
    if result.error:
...
def render_hotel_checker() -> None:
    st.markdown(HOTEL_STYLES, unsafe_allow_html=True)
    st.markdown(
        """
...
""",
    ...
    with c3:
        nights = st.number_input(
            "Nights", min_value=1, max_value=14, value=DEFAULT_NIGHTS, key="hotel_nights_input"
        )
    with c4:
        rooms = st.number_input(
            "Rooms", min_value=1, max_value=5, value=DEFAULT_ROOMS, key="hotel_rooms_input"
        )
    with c5:
        adults = st.number_input(
            "Adults", min_value=1, max_value=8, value=DEFAULT_ADULTS, key="hotel_adults_input"
        )
    ...
    if check_now:
        with st.spinner("Opening Omni booking calendar…"):
            _run_check(hotel_id, hotel_name, arrival, int(nights), int(rooms), int(adults))
        st.rerun()
    ...
    refresh_secs = 86400 if st.session_state.get("hotel_auto_check") else None
    ...
    def auto_check_fragment():
        ...
        if result is not None:
            available = (
                ...
                else result.get("available", False)
            )
        st.toast(
